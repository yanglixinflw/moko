$(function(){
    ...
    for(let i=0;i<li.length;i++)
        li.eq(i).click(function (){
            $(".index_xinwen_con .left .left_con").eq(i).css({"opacity":"1","z-index":"100"}).siblings("div").css({"opacity":"0","z-index":"50"});
            $(".index_xinwen_con .left ul li").eq(i).css({
                'background': 'rgba(0,0,0,0.9)',
                'filter': 'progid:DXImageTransform.Microsoft.gradient(startColorstr=#e5ffffff,endColorstr=#e5ffffff)'
            }).siblings('li').css({
                'background': 'rgba(0,0,0,0.5)',
            ...
            $(".index_xinwen_con .right .right_bottom ul li a").eq(i).css({
                'color': '#0f88eb'
            });
		});
